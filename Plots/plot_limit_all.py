...
ol = '/eos/user/z/zhjie/CMSSW_10_2_13/src/flashggFinalFit/Plots/check'
...
def plotUpperLimits(labels,values,resultType):
    ...
    mass=[1000,1200,1400,1600,1800,2000,2400,2500,2600,2800,3000,3500,4000]
    nexp=len(mass)
    yellows={}
    greens={}
    medians={}
    ...
    for i in mass:
        ...
        for j in range(N):
            file_name = '/eos/user/z/zhjie/CMSSW_10_2_13/src/flashggFinalFit/Datacard/final/higgsCombineall_'+str(i)+'_M%s.AsymptoticLimits.mH125.root'%(values[i][j])
            ...
            if limit is None or len(limit) < 3:
                continue
            ...
            x_vals.append(value[j])
            y_vals_yellow_up.append(limit[4] * HHWWgg_factor * factor)  # + 2 sigma
            y_vals_yellow_down.append(limit[0] * HHWWgg_factor * factor)  # - 2 sigma
            y_vals_green_up.append(limit[3] * HHWWgg_factor * factor)  # + 1 sigma
            y_vals_green_down.append(limit[1] * HHWWgg_factor * factor)  # - 1 sigma
            y_vals_median.append(limit[2] * HHWWgg_factor * factor)  # median


        if len(x_vals) > 3:  
            spline_yellow_up = UnivariateSpline(x_vals, y_vals_yellow_up, s=0)
            spline_yellow_down = UnivariateSpline(x_vals, y_vals_yellow_down, s=0)
            spline_green_up = UnivariateSpline(x_vals, y_vals_green_up, s=0)
            spline_green_down = UnivariateSpline(x_vals, y_vals_green_down, s=0)
            spline_median = UnivariateSpline(x_vals, y_vals_median, s=0)

          
            x_vals_smooth = np.linspace(min(x_vals), max(x_vals), 20)  

            for j, x in enumerate(x_vals_smooth):
                yellows[i].SetPoint(j, x, spline_yellow_up(x))
                yellows[i].SetPoint(2 * len(x_vals_smooth) - 1 - j, x, spline_yellow_down(x))
                greens[i].SetPoint(j, x, spline_green_up(x))
                greens[i].SetPoint(2 * len(x_vals_smooth) - 1 - j, x, spline_green_down(x))
                medians[i].SetPoint(j, x, spline_median(x))
        else:
            for j in range(N):
                yellows[i].SetPoint(j, value[j], y_vals_yellow_up[j])
                yellows[i].SetPoint(2 * N - 1 - j, value[j], y_vals_yellow_down[j])
                greens[i].SetPoint(j, value[j], y_vals_green_up[j])
                greens[i].SetPoint(2 * N - 1 - j, value[j], y_vals_green_down[j])
                medians[i].SetPoint(j, value[j], y_vals_median[j])
...
#    frame.GetYaxis().SetTitle("95% upper limit on #sigma #times BR / (#sigma #times BR)_{SM}")
    ...
    for i in mass:
        frame.GetXaxis().SetLimits(min(values[i]),max(values[i])+300)
...
def getMXMY(filename):
    mx_my = re.search("data_MX-(\d+)_MY-(\d+)", filename)
...
def main():
    ...
    labels={}
    for j in mass:
        sigFiles=glob.glob('/eos/cms/store/group/phys_higgs/cmshgg/zhjie/output/output_'+str(j)+'/data_*.parquet')
        value=[]
        label=[]
