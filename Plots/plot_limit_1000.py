...
def plotUpperLimits(labels,values,resultType):
    ...
    for i in mass:
        ...
        limit_values={}
        for j in range(N):
            file_name = '/eos/user/z/zhjie/CMSSW_10_2_13/src/flashggFinalFit/Datacard/All/higgsCombineall_1000_M%s.AsymptoticLimits.mH125.root'%(values[i][j])
            print("file: ",yellows[i])
            print(type(yellows[i]))
            limit= getLimits(file_name)
            # print(limit)
            if(resultType == "bbgg"): HHWWgg_factor = 1
            elif(resultType == "HY"): HHWWgg_factor = 1/0.0023
            ...
            #     f.write("Ymass: "+str(values[i][j])+" "+str(limit[2]*0.0023)+"\n")
            idex=str(values[i][j])
            limit_values[idex]=[limit[2]*0.0023]
            # print("Ymass",values[i][j],limit[2]*0.0023)
        # HHWWgg_qqlnu_factor = campaignBRdict[args.campaign]
        df = pd.DataFrame(limit_values)
        df.to_html('X1000.html')
        factor=factor/10
    # vals=vals*0.0023
    W = 800
    H  = 600
...
#    frame.GetYaxis().SetTitle("95% upper limit on #sigma #times BR / (#sigma #times BR)_{SM}")
    ...
    for i in mass:
        frame.GetXaxis().SetLimits(min(values[i]),max(values[i]))
...
#    legend.AddEntry(green, "AsymptoticLimits CL_{s} #pm 2 std. deviation",'f')
    ...
    # args.lumiRescale
    c.SaveAs("%s_%s_%s_1000_UpperLimit.pdf"%(outFile,args.lumiRescale,resultType))
    c.SaveAs("%s_%s_%s_1000_UpperLimit.png"%(outFile,args.lumiRescale,resultType))
    c.SaveAs("%s_%s_%s_1000_UpperLimit.C"%(outFile,args.lumiRescale,resultType))
    c.Close()
...
def main():
    ...
    labels={}
    for j in mass:
        sigFiles=glob.glob('/eos/cms/store/group/phys_higgs/cmshgg/zhjie/output/output_1000/opt/data/*cat0*parquet')
        value=[]
        label=[]
        for f in sigFiles:
            y=getMXMY(f)
